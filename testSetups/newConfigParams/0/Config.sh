...
#GRADIENTS_GREEN_GAUSS                   # original (now depreciated) graddient estimate, reduced hydro scheme to first order
...
FOF_SECONDARY_LINK_TYPES=1         # 2^type for the types linked to nearest primaries
...
FOF_GROUP_MIN_LEN=100               # default is 32
...
TRACER_MC=2                   # Monte Carlo tracer particles (=1 to enable, >=2 to output as that partType)
...
##TRACER_MC_STORE_WHAT=1+2+4+8+16+32+1024+2048+4096     # bit mask for quantities to store (see allvars.h for bitmask)
...
#SIDM_READ_ICS                         #read SIDM information from ICs 
...
#--------------------------------------- Sinks
SINK_PARTICLES
#SINK_PARTICLES_COMOVING
DUMP_SINK_PARTICLE_INFO
SINK_PARTICLES_SKIM_CELL_MASS
SINK_PARTICLES_FEEDBACK
#SINK_PARTICLES_NO_JEANS
#SINK_PARTICLES_FIXED_STARTING_MASS 
...
#SGCHEM_RT                              # main switch for the radiation transfer

#---- SPRAI - SimpleX Photon Radiation 
#SIMPLEX                        # SimpleX main switch   
#
#SX_CHEMISTRY=3                 # chemistry network: 3) SGChem, 4) FiBY                               
#                           
#SX_NDIR=84                     # number of directional bins (sx_direction_bins.h)  
#SX_SOURCES=5                   # source of radiation 4 = stars; 5 = sinks; 10+ = test sources (sx_test_sources.h)
##SX_NUM_STEPS=0                 # number of radiation substeps (0/undefined = evolve photons until complete attenuation)
#SX_NUM_ROT=5                   # number of direction base rotations per RT (0/undefined = static base)  
#SX_CLEAR_PHOTONS_BEFORE_RUN    # number of photons in all cells is set to zero after each run ( undefined = keep photons on the grid)  
#SX_FLOAT_PRECISION             # do photon transfer in float precision (undefined = use double)                             
#SX_IMAGE                       # produce Arepo images of the H photoionization rate (RIH) and the associated heating rate (HRIH) (slices and projections)
#SX_IMAGE_ALL                   # produce multi-dimensional Arepo image with all ionization rates (slices only)
#
##SX_DISPLAY_STATS               # display statistics about RT in the terminal  
#
#SX_RECOMBINE
#SX_DISPLAY_TIMERS
