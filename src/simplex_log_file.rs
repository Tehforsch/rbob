...
impl SimplexLogFile {
    ...
    pub fn get_average_ionization_over_time(&self) -> Result<Vec<(f64, f64)>> {
        let re = Regex::new("Time ([0-9.+]+): Av. H ionization: ([0-9.+]+)").unwrap();
        re.captures_iter(&self.get_contents()?)
            .map(|cap| {
                let time = cap.get(1).unwrap().as_str().parse::<f64>()?;
                let ionization = cap.get(2).unwrap().as_str().parse::<f64>()?;
                Ok((time, ionization))
            })
            .collect()
    }
